...
<form class="row g-3" method="POST" action="{{route('atualizar.produto', $findProduto->id)}}">
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Atualizar Produto: {{ $findProduto->id }}</h1>
    </div>
    <div class="md-3">
      <label for="nome" class="form-label">Nome</label>
      <input type="text" value="{{ isset($findProduto->nome) ? $findProduto->nome : old('nome') }}" class="form-control @error('nome') is-invalid @enderror" name="nome" id="nome">
      @if ($errors->has('nome'))
